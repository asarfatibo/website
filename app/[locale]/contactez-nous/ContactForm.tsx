...
export function ContactForm({ t }: { t: Dictionary["contact"]["form"] }) {
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!objet.trim() || !email.trim() || !message.trim()) return;
    setState("sending");

    const subject = encodeURIComponent(objet.trim());
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-8 space-y-5">
      <div className="space-y-1.5">
        <label className="text-sm font-medium text-ink" htmlFor="objet">
          {t.subjectLabel} <span className="text-blue">*</span>
        </label>
        <input
          id="objet"
          type="text"
